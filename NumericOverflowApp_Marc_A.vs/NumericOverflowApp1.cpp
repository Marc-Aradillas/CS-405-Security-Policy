...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        if (result > std::numeric_limits<T>::max() - increment) // Checks If 'result' exceeds the maximum limit for type T if adding 'increment'
        {
            std::cout << "\t[ERROR] Overflow detected! Returning max value.\n";
            return std::numeric_limits<T>::max(); // Handling Overflow; returns max value
        }
        result += increment;
    }
    return result;
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result < std::numeric_limits<T>::min() + decrement) // Checks If 'result' goes below minimum limit for type T if subtracting 'decrement'
        {
            std::wcout << "\t[ERROR] Underflow detected! Returning min value.\n";
            return std::numeric_limits<T>::min(); // Handling underflow; returns min value
        }
        result -= decrement;
    }
...
void test_overflow()
{
    ...
    // ===== Added Code : verifying result after addition ===== //
    ...
    // Checks if 'result' has reached maximum value after operation
    if (result == std::numeric_limits<T>::max())
    {
        std::cout << "\t[ERROR] Overflow occurred! Operation failed.\n";
    }
    else
    {
        std::cout << +result << std::endl; // display result if overflow not detected
    }
}
...
void test_underflow()
{
    ...
    // ===== Added code: Verify underflow after subtraction ===== //
    ...
    // Check if 'result' has reached the minimum value after operation
    if (result == std::numeric_limits<T>::min())
    {
        std::cout << "\t[Error] Underflow occurred! Operation failed.\n";
    }
    else 
    {
        std::cout << +result << std::endl; // display result if underflow not detected
    }
}
